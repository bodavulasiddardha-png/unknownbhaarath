...
function validateSlide(slide, category) {
  ...
  const REFUSAL_PHRASES = [
    'cannot create', 'cannot generate', 'can not create', 'unable to',
    'no data', 'no career', 'no salary', 'no hiring', 'no skill',
    'no relevant', 'not available', 'does not contain', 'mismatch',
    'incompatible', 'not enough', 'insufficient', 'no information',
    'category incompatible', 'world news', 'unrelated'
  ];
...
export async function generateSlide(category, topic, articles) {
  ...
  try {
    ...
    if (verdict === 'fail' || confidence < 85) {
      ...
      const slide = validateSlide(normalizeSlide(await claudeStorytellingFact(category, f2, s2), category, topic), category);
